...
!	this module must also be declared in the calling routine.
	...
	USE MOD_SAIL
	USE MOD_flag_util
    USE MOD_staticvar!, ONLY: cts, cto, ctscto, ddb, ddf, dob, dof, dso, ko, ks, sdb, sdf, sob, sof, sumint, &
    !sb, sf, vb, vf, w, m, m2, att, sigb, rinf, lidf, e1, e2, rinf2, re, denom, Ps, Qs, Pv, Qv
    IMPLICIT NONE
...
IF (init_completed) THEN
	...
	flag(1) = .true.!delta_geom
	flag(2)	= .true.!delta_lidf
	flag(3) = .true.!delta_geom.or.delta_lidf
	flag(4) = .true.!flag(3).or.delta_leaf
	flag(5) = .true.!flag(4).or.delta_lai.or.delta_soil
	flag(6) = .true.!flag(3).or.delta_lai.or.delta_hot
	flag(7) = .true.!delta_leaf
ENDIF
