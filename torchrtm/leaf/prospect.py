...
@torch.jit.script
def plate_torch_both(trans: torch.Tensor, r12: torch.Tensor, t12: torch.Tensor,
                     t21: torch.Tensor, r21: torch.Tensor, xp: torch.Tensor,
                     yp: torch.Tensor, N: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Plate model to compute multiple scattering in leaf layers.
...
# 函数 2: 仅计算反射率
@torch.jit.script
def plate_torch_rho_only(trans: torch.Tensor, r12: torch.Tensor, t12: torch.Tensor,
                         t21: torch.Tensor, r21: torch.Tensor, xp: torch.Tensor,
                         yp: torch.Tensor, N: torch.Tensor) -> torch.Tensor:
    """
    Plate model to compute multiple scattering in leaf layers.
    ...
    vb_diff = vbNN - vbNNinv
    s1 = (t12 * t21 * trans) / denom * t90 * vb_diff
    s3 = va * vbNN - vainv * vbNNinv - r90 * vb_diff

    rho = ra + s1 / s3

    return rho
def plate_torch(trans, r12, t12, t21, r21, xp, yp, N,print_both= True):
    if print_both:
        return(plate_torch_both(trans, r12, t12, t21, r21, xp, yp, N))
        
    else:
        return(plate_torch_rho_only(trans, r12, t12, t21, r21, xp, yp, N))
...
def prospectd(traits, N, CoefMat=None, alpha=40.0, print_both=True, device='cpu'):
    ...
    alpha_vals = (traits.T / N).T
    k_pd = torch.matmul(alpha_vals, th_CoefMat.T)
    trans_rtm = (1 - k_pd) * torch.exp(-k_pd) + k_pd**2 * exp1_pytorch(k_pd) #exp1
