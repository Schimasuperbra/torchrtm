...
def call_torch_batched(xb, xq, k, distance_order=2, batch_size=64):
    ...
    for i in range(0, n, batch_size):
        ...
        print(xq_batch.shape,xb.shape)
        bs_scores = torch.cdist(xq_batch, xb, p=distance_order)  # Compute distances
        print(bs_scores.shape)
        topk_indices.append(bs_scores.topk(k=k, dim=1, sorted=True, largest=False))  # Get top-k indices
        print(bs_scores.topk(k=k, dim=1, sorted=True, largest=False)[1])
    return torch.cat(topk_indices, dim=0)  # Concatenate results from all batches
def call_torch_batched(xb, xq, k, distance_order=2, batch_size=64):
    """
    Computes the k-nearest neighbors in batches to handle large datasets efficiently.
